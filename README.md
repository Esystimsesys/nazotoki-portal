...
| チーム管理 | チームの登録・削除、ログインコードの確認と再発行、最終ログイン日時（未ログインのチーム）の確認 |
| ダッシュボード | ランキング、問題別サマリ、チーム別のドリルダウン、**統合結果レポート（Excel）出力** |
| 大画面表示 | 会場のプロジェクタへ投影する専用画面。ランキングを伏せた状態からボタンで結果発表でき、全画面表示にも対応 |
...
6. 「■ 終了」で受付終了 → 大画面表示の「結果を表示」で結果発表
7. ダッシュボードから結果レポートを出力（大会概要・順位・問題×チーム・問題別集計・回答履歴を1つのExcelに収録）
