...
| createdAt | 作成日時 |
| note | 管理者向けメモ（任意。例: メンバー名の控え、受付での申し送り）。参加者には表示しない |
| lastLoginAt | 最終ログイン日時（任意）。共有コードでのログイン成功時に更新する。属性が無ければ一度もログインしていない |
...
- 同じ4桁コードが、**他のProblem（有効・無効を問わず。自分自身は除く）**のAnswerPatternに既に存在する場合はエラーとし、登録を拒否する。複数問題が同時に有効化されうる仕様上、同時に有効なコードが重複すると参加者の回答がどの問題への回答か一意に定まらなくなるため。無効な問題も対象に含めるのは、無効なうちに重複を登録できてしまうと、あとで有効化しようとした瞬間に初めて弾かれ、イベント進行中の事故になるため。CSV一括取込では、取込データ全体を横断して同一チェックを行い、重複行はエラー一覧として提示し、修正するまで取込を確定しない。
